...
class TestParameters(unittest.TestCase):
    ...
    def test_iso_timestamp_from_iso(self):
        ...
        p = botocorev063p.parameters.TimestampParameter(op, name='paramname',
                                                   type='timestamp')
        d = {}
        ts = '2012-10-12T00:00'
        ...
        with self.assertRaisesRegexp(botocorev063p.exceptions.ValidationError,
                                     'timestamp:paramname'):
    ...
    def test_iso_timestamp_from_epoch(self):
        op = FakeOperation('iso8601')
        p = botocorev063p.parameters.TimestampParameter(op, name='foo')
        d = {}
        iso = '2013-04-11T23:23:15'
        epoch = 1365722595
        p.build_parameter_query(epoch, d)
        self.assertEqual(d['foo'], iso)
        with self.assertRaises(botocorev063p.exceptions.ValidationError):
            p.build_parameter_query(value='not a date string',
                                    built_params=d)
    ...
    def test_integer(self):
        p = botocorev063p.parameters.IntegerParameter(None, name='foo')
        d = {}
    ...
    def test_integer_range(self):
        ...
        with self.assertRaises(botocorev063p.exceptions.ValidationError):
            p.build_parameter_query(value=8.4,
                                    built_params=d)
        with self.assertRaisesRegexp(botocorev063p.exceptions.RangeError,
                                     'foo'):
            p.build_parameter_query(value=100,
                                    built_params=d)

    def test_float(self):
        p = botocorev063p.parameters.FloatParameter(None, name='foo')
        d = {}
    ...
    def test_float_range(self):
        ...
        self.assertEqual(d['foo'], 9.0)
        with self.assertRaises(botocorev063p.exceptions.RangeError):
            p.build_parameter_query(value=100.0,
                                    built_params=d)


    def test_boolean(self):
        p = botocorev063p.parameters.BooleanParameter(None, name='foo')
        d = {}
    ...
    def test_list_fails_member_validation(self):
        p = botocorev063p.parameters.ListParameter(operation=None, name='foo',
                                              members={'type': 'string'})
        d = {}
    ...
    def test_list_with_xmlname(self):
        ...
        p = botocorev063p.parameters.ListParameter(
    ...
    def test_type_blob_validation(self):
        p = botocorev063p.parameters.BlobParameter(
...
class TestStructureParamaters(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.p = botocorev063p.parameters.StructParameter(
            operation=None, name='Foo',
    ...
    def test_struct_type_validation(self):
        ...
        with self.assertRaisesRegexp(
                botocorev063p.exceptions.ValidationError,
    ...
    def test_struct_unknown_keys_validation(self):
        ...
        with self.assertRaises(botocorev063p.exceptions.UnknownKeyError):
